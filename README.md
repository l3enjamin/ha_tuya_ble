...
The codebase has undergone a major internal refactoring to align with Home Assistant 2024.x best practices:

- **Runtime Data Pattern**: All platform components now use `entry.runtime_data` instead of `hass.data[DOMAIN]`
- **Type Safety**: Introduced `TuyaBLEConfigEntry = ConfigEntry[TuyaBLEData]` for better IDE support and type checking
- **Modern Patterns**: Updated task creation and async patterns throughout

See [CHANGELOG.md](CHANGELOG.md) for detailed migration notes.

### Project Status

This integration is maintained by [@airy10](https://github.com/airy10) and the community. Contributions are welcome!

**Architecture**: The codebase follows Home Assistant's modern integration patterns (as of 2024.x).
